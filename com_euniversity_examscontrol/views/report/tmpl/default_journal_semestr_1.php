...
$query->leftjoin('#__euniversity_cat_discipline as ct_dis on ct_dis.id = t.discipline');
...
$query->select('IFNULL(e.semestr,(p.course*2-1)) as semestr');
...
$query->select('e.discipline as discipline');
$query->select('(CASE WHEN c_dis.id<>0 THEN c_dis.description ELSE ct_dis.description END) as discipline_name');
$query->select('(CASE WHEN c_dis.id<>0 THEN c_dis.id ELSE t.discipline END) as discipline_id');
...
$query->having('mod(semestr,2)=0');
$query->having('mod(semestr,2)=1');
...
$query->select('r_dis.cat_id as discipline_id');
$query->group('p.uid');
$query->group('l.discipline');

$query->where('mod(l.semestr,2)=1');
